...
#
#                   Figure 3: Pollen accumulation rate diagram
#
#                                  Petr Kuneš
#                                     2025
#
...
# pdf("data_out/Fig_3.pdf", width = 10, height = 6)
...
rp2.1 <- riojaPlot(tilia_PAR(CER_pollen, list("TRSH", "UPHE"), list("pollen"), piwot.wider = T) %>% select(unlist(taxalist[1])), 
                   CER_pollen$metadata %>% rename(`Age [cal. years BP]` = `#Chron1`), 
                   xlabels = "Pinus\nPAR",
                   # labels.italicise = T,
                   yvar.name = "Age [cal. years BP]", scale.percent = F, xRight = shift,
...
usr <- par("usr")
mtext(expression("grains/cm"^2*"/year"), side = 1, line = 4, cex = 0.7, at = usr[1] + 0.05 * (usr[2] - usr[1]))
# dev.off()
