...
# pdf(file = "data_out/Fig_CER_Index.pdf", width = 10, height = 6)
...
rp1 <- riojaPlot(CER_perc_EPD %>% select(-depth) %>% ungroup(), CER_EPD$metadata %>% rename(`Age cal. BP` = `#Chron1`),
                 groups = CER_groups,
                 col.group = c("#117733", "#ddcc77", "#CC6677", "#882255"),
                 plot.sec.axis = T, yvar.name = "Age cal. BP", 
